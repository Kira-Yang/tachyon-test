...
public class TestDFSIO implements Tool {
  ...
  public static class WriteMapper extends IOStatMapper {
    ...
    public Closeable getIOStream(String name) throws IOException {
      // create file
      String writeType = getConf().get("test.io.write.type");
      OutputStream out;
      if (writeType != null) {
	    String uri = getConf().get("fs.defaultFS");
	    TachyonURI tUri = new TachyonURI(uri);
	    TachyonFS tfs = TachyonFS.get(tUri);
	    TachyonURI path = new TachyonURI(new Path(getDataDir(getConf()), name).getName());
	    tfs.createFile(path);
	    TachyonFile file = tfs.getFile(path);
	    WriteType type = WriteType.valueOf(writeType);
	    out = file.getOutStream(type);
	  } else {
  ...
  private void sequentialTest(FileSystem fs, 
    ...
    for(int i=0; i < nrFiles; i++) {
        ioer.map(new Text(getFileName(i)), new LongWritable(fileSize), output, Reporter.NULL);
    }
  }
    ...
    for (int i = 0; i < args.length; i++) {       // parse command line
      if (args[i].startsWith("-read")) {
        ...
        resFileName = args[++i];
    	  ...
    	  config.set("test.build.data", args[++i]);
      } else if (args[i].equals("-wt")) {
    	  config.set("test.io.write.type", args[++i]);
      } else {
...
//    config.setStrings("test.io.fs.url", fsUrl);
    LOG.info("fsUrl=" + fsUrl);
    if (fsUrl != null) {
        config.set("fs.defaultFS", fsUrl);
	}
//    FileSystem fs = fsUrl == null ? FileSystem.get(config) : FileSystem.get(URI.create(fsUrl), config);
